class Author:
    ...
    def magazines(self):
        ...
        sql = """SELECT DISTINCT magazines.id, magazines.name, magazines.category FROM magazines
                 INNER JOIN articles ON articles.magazine_id = magazines.id WHERE articles.author_id = ?"""
        rows = self.cursor.execute(sql, (self.id,)).fetchall()
